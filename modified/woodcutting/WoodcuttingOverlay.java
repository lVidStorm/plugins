...
public class WoodcuttingOverlay extends OverlayPanel {
    ...
    @Inject
    WoodcuttingOverlay(WoodcuttingPlugin plugin, WoodcuttingConfig config)
    {
        super(plugin);
        this.config = config;
    ...
    public Dimension render(Graphics2D graphics) {
        try {
            ...
            if (config.distanceToStray() < 21) {
                LocalPoint lp =  LocalPoint.fromWorld(Microbot.getClient(), WoodcuttingScript.initPlayerLoc(config));
                if (lp != null) {
