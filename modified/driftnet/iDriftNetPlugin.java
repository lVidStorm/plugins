...
import static net.runelite.client.plugins.PluginDescriptor.Default;
import static net.runelite.client.plugins.PluginDescriptor.Mocrosoft;

@PluginDescriptor(
        name = Default + "iDrifnet",
        description = "Microbot idrifnet plugin",
        tags = {"fishing", "microbot", "hunter", "drift", "net"},
        enabledByDefault = false
)
@Slf4j
public class iDriftNetPlugin extends Plugin {
    static final String CONFIG_GROUP = "idriftnet";
    ...
    private static final String CHAT_FISH_CARRY_ON = "The fish look at you strangely and carry on their way.";
    private static final String CHAT_BANK_PUFFERFISH = "Your bank could not hold your catch.";
    ...
    @Inject
    private iDriftNetConfig config;

    @Inject
    private OverlayManager overlayManager;

    @Inject
    private iDriftNetOverlay overlay;
    ...
    static boolean successfulChase = false;
    static boolean newChatMessage = false;
    static boolean netBanked = false;
    ...
    @Provides
    iDriftNetConfig provideConfig(ConfigManager configManager) {
        return configManager.getConfig(iDriftNetConfig.class);
    }

    @Inject
    private iDriftNetOverlay driftnetOverlayI;

    @Inject
    iDriftNetScript driftnetScriptI;


    @Override
    protected void startUp() throws AWTException {
        if (overlayManager != null) {
            overlayManager.add(driftnetOverlayI);
        }
        ...
        driftnetScriptI.run(config);
    }

    protected void shutDown() {
        driftnetScriptI.shutdown();
        reset();
        overlayManager.remove(driftnetOverlayI);
    }
    @Subscribe
    ...
    private void updateDriftNetVarbits()
    {
        ...
        {
            iDriftNetStatus status = iDriftNetStatus.of(client.getVarbitValue(net.getStatusVarbit()));
    ...
    public void onChatMessage(ChatMessage event)
    {
        ...
        if (event.getType() == ChatMessageType.SPAM && event.getMessage().equals(CHAT_PRODDING_FISH))
        {
            ...
            else
            {
                ...
                if (!newChatMessage) { newChatMessage=true; }
                successfulChase = true;
                armInteraction = true;
            }
        } else if (event.getType() == ChatMessageType.SPAM && event.getMessage().equals(CHAT_FISH_CARRY_ON)) {
            if (!newChatMessage) { newChatMessage=true; }
            successfulChase = false;
        } else if (event.getType() == ChatMessageType.SPAM && event.getMessage().equals(CHAT_BANK_PUFFERFISH)) {
            if(!netBanked) {System.out.println("Detected banking"); netBanked = true; }
        }
    }
