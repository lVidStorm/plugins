...
import net.runelite.client.plugins.microbot.storm.ogPlugins.ogBlastFurnace.enums.Bars;
...
public class ogBlastFurnaceScript extends Script {
    ...
    private int refillCofferAmmount;//TODO oh. it might be like the runecrafting script. coffer data is never used. same for most of the config values?? means I'll have to set that up myself. @_@
    ...
    private int getStamEffect() {return Microbot.getClientThread().runOnClientThread(() -> Microbot.getClient().getVarbitValue(Varbits.STAMINA_EFFECT));}//TODO oh. I think I like this better than what I have. assuming it works
    private void stamPotUp() {
        //TODO maybe touch this up later~
    ...
    //TODO I saw.
    //TODO recap, wait when banking, fill coffer,
    ...
    private void retrieveBars() {
        ...
        sleep(200,400);
        sleep(300,600);
        if(getBFDispenserState() == 1){sleepUntil(() -> getBFDispenserState() == 2 || getBFDispenserState() == 3);}//TODO we'll definitely need this for later
        ...
        if (Rs2GameObject.interact(9092, "Take")) {
            ...
            Rs2Keyboard.keyPress(KeyEvent.VK_SPACE);//TODO I'll just leave it in for now since it won't actually wait that long, that's just a time-out limit
            sleepUntil(() -> !Rs2Inventory.hasItem("gold bar"),60000);//TODO I think the original script maker got confused as shit here too, as the wait time is 60000 xD "Why isn't it detecting the gold bars in my inventory?!!"
        }//TODO because of the inverse ! operator. xd
        if (Rs2Widget.hasWidget("Gold bar")) {//TODO correct
            Rs2Keyboard.keyPress(KeyEvent.VK_SPACE);//TODO this isn't even necessary, we can just run away when we have our items.
        }
        sleep(delaytest);
...
//            Rs2Bank.scrollTo(Rs2Widget.findWidget(barName())); // FIXME: HONESTLY THIS IS SO OLD IT DOESN'T EVEN WORK
            ...
            Rs2Bank.withdrawAll("Gold ore");//TODO ah. that's why.
    ...
    private void depositOverflow() {
        ...
        Rs2Bank.depositAll(barSelected.getBarID());
        sleepUntil(() -> !Rs2Inventory.hasItem(barSelected.getBarID()),1800);
        Rs2Bank.depositAll(barSelected.getPrimaryOre());
        sleepUntil(() -> !Rs2Inventory.hasItem(barSelected.getPrimaryOre()),1800);
        sleep(50,80);
    ...
    private void setSettings(ogBlastFurnaceConfig config){
        ...
        this.delaytest = config.delaytest();
    }
}