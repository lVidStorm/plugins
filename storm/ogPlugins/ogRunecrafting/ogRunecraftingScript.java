package net.runelite.client.plugins.microbot.storm.ogPlugins.ogRunecrafting;
...
public class ogRunecraftingScript extends Script {
    ...
    private void teletoCraftingGuild() {
        ...
        sleepUntil(() -> Rs2Tab.getCurrentTab() == InterfaceTab.EQUIPMENT);
        sleep(100, 150);
        Rs2Widget.clickWidget(25362448);
    }
    private void teletoFireAlter() {
        Rs2Tab.switchToEquipmentTab();
        sleepUntil(() -> Rs2Tab.getCurrentTab() == InterfaceTab.EQUIPMENT);
        sleep(200, 300);
        Rs2Widget.clickWidget(25362456);
    }
    ...
    private void grabRunes() {
        ...
        if(!Rs2Bank.isOpen()){
            ...
            if(!Rs2Widget.getWidget(786435).getText().contains("runecraft")){
                while(!Rs2Widget.isWidgetVisible(786442,3)){Rs2Widget.clickChildWidget(786442,0);}
                waitNextTick();
