...
import net.runelite.client.plugins.microbot.storm.modified.gotr.data.CellType;
...
//TODO adapt this method here~
    ...
    private boolean leaveLargeMine() {
        if (isInLargeMine()) {
            ...
            state = zGotrState.LEAVING_LARGE_MINE;
            return true;
        }
        return false;
    }
    ...
    public static List<GameObject> getAvailableAltars() {
        ...
        return Rs2GameObject.getGameObjects().stream()
                .filter(x -> {
                    ...
                    if (zGotrScript.guardianPortalInfo.get(x.getId()).getRequiredLevel()
                            > Microbot.getClient().getBoostedSkillLevel(Skill.RUNECRAFT)) {
                        return false;
                    }
                    if (zGotrScript.guardianPortalInfo.get(x.getId()).getQuestState() != QuestState.FINISHED) {
                        return false;
                    }
