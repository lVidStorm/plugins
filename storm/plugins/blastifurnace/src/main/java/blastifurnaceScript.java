...
public class blastifurnaceScript extends Script {
    ...
    public boolean run(blastifurnaceConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (!super.run()) return;
                if(System.currentTimeMillis()-lastXpDrop>30000*timeoutMultiplier){
    ...
    private void retrieveBars(){
        ...
        //TODO change this in case the player levels up and stops moving


        if(this.isRunning() && config.getBars()!=Bars.GOLD_BAR && config.getBars()!=Bars.SILVER_BAR && config.getBars()!=Bars.STEEL_BAR) {
        ...
        if(!config.useCoalBag() || config.getBars()==Bars.GOLD_BAR || config.getBars()==Bars.SILVER_BAR || config.getBars()==Bars.IRON_BAR) {
            ...
            if(config.getBars()==Bars.STEEL_BAR){
                if(secondaryTripCounter > 1) {
                    ...
                    actions(interact, 9100); customSleep("high");
